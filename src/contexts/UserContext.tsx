...
export const UserContextProvider: React.FC<Props & Record<string, unknown>> = ({
    ...
    onCompleted: ({ getUserDashboard }) => {
      ...
      if (getUserDashboard.dashboard) {
        if (
          ...
          const dashboardPets = getUserDashboard.dashboard.ownerships.items.map(
            (item) => ({
              ...item!.pet,
        ...
        if (getUserDashboard.dashboard.reports?.items) {
          setReports(
            getUserDashboard.dashboard.reports.items as MinReportFragment[]
          );
        }
      }
      return;
  ...
  return (
    <UserContext.Provider value={value}>
      ...
      <MainBody className={!visible ? "noUserMenu" : ""}>{children}</MainBody>
      <MainMenu
        open={menuOpen}
        onClose={() => {
          setMenuOpen(false);
