...
import { useGetPetWeightStatsQuery } from "../../pets/operations/__generated__/getPetWeightStats.generated";
import { useCreatePetWeightMutation } from "../../pets/operations/__generated__/createPetWeight.generated";

export const ShelterPetWeightStats: React.FC = () => {
	...
	const [petRealId, setPetRealId] = useState<string>();
	...
	const [getShelterPet] = useGetShelterPetLazyQuery({
		...
		onCompleted: ({ getShelterPet }) => {
			...
			if (sp?.pet?.id) setPetRealId(sp.pet.id);
		},
	});
	...
	const { data, loading, refetch } = useGetPetWeightStatsQuery({
		...
		variables: { pet_id: petRealId as string, period },
	});
	...
	const submit = () => {
		...
		if (!weight_kg || weight_kg <= 0 || !petRealId) return;
		createWeight({ variables: { data: { pet_id: petRealId, weight_kg } } });
	};
