        ...
        if (g.mode === "pinch" && pointers.current.size === 2) {
            ...
            const rot =
                ...
                (Math.abs(angleDeltaDeg) > ROT_THRESHOLD_DEG
                    ? angleDeltaDeg -
                      Math.sign(angleDeltaDeg) * ROT_THRESHOLD_DEG
                    : 0);
            setView((v) => {
                const target = Math.min(
                    MAX_SCALE,
                    Math.max(
                        MIN_SCALE,
                        (g.startScale! * dist) / (g.startDist || 1),
                    ),
                );
                const k = target / v.scale;
            ...
            {zoomOpen && (
                ...
                >
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            applyZoomPercent(parseInt(e.target.value))
                        }
                    />
                    <ZoomReset
                        type="button"
